...
export class InicioSesionComponent {
  ...
  iniciarSesion(): void {
    ...
    this.authService.login(solicitud).pipe(
      finalize(() => this.loaderService.desactivar()),
    ).subscribe({
      next: () => this.manejarInicioSesionCorrecto(),
      error: (error: HttpErrorResponse) => this.manejarInicioSesionError(error)
    });
  }
  ...
  obtenerMensajeError(status: number): string {
    if (status === 401) return 'CURP o contraseña incorrectos, favor de validar.'
