...
@Component({
  ...
  providers: [RegistroService, NotificacionesService],
...
export class RegistroInicialComponent implements AfterViewInit {
  ...
  crearRegistroForm(): FormGroup {
    return this.formBuilder.group({
      contrasena: ['', [Validators.required, Validators.pattern(this.REGEX_PASS), Validators.minLength(8),
  ...
  irSiguienteSeccion(): void {
    ...
    this.registroService.verificarCodigo(verificar)
    .subscribe({
        next: (resp) => { 
          console.log(resp)
        ...
        error: (err) => {this.caducado = err.error.caducado; this.manejoRegistroInicialError(err)},
        complete: () => { }
      });    
  }
  ...
  nuevocodigo(): void{
    ...
    this.registroService.solicitarReferenciaCodigo(codigo)
    .pipe(
      finalize(() => this.loaderService.desactivar() ),
    ...
    .subscribe({
      next: (resp) => { this.mostrarAlertaRegistroInicial("El codigo fue solicitado exitosamente, revisar tu correo electronico"); console.log(resp) },
      error: (err) => { this.mostrarAlertaRegistroInicialError(err.error.message) }
    })
  }
  get f() {
