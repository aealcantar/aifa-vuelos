...
export class NotificacionesService {
  ...
  restaurarPassword(resetRequest: ResetPassRequest): Observable<any> {
    return this.httpClient.post<any>(`${environment.api.notificaciones}${this._url_recuperar_pass}`, resetRequest, { observe: 'response' });
  }

  verificarCodigo(codeRequest: CodeRequest): Observable<HttpResponse<CodeResponse>> {
    return this.httpClient.post<CodeResponse>(`${environment.api.password}${this._url_verificar_codigo}`, codeRequest, {
      observe: 'response',
    });
  }

  cambiarPassword(changePassRequest: ChangePassRequest): Observable<any> {
    return this.httpClient.post<any>(`${environment.api.password}${this._url_new_pass}`, changePassRequest, { observe: 'response' });
  }
  ...
  cerrarNotificacion(id: number) {
    return this.httpClient.post<any>(
      `${environment.api.notificaciones_usuario}${this._url_read_notify}`,
      { ID_NOTIFICACION: id },
      { observe: 'response' },
    );
  }
